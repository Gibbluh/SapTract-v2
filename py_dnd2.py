...
# I will replace the 5 cards with a mapping.
# It's better to just extract their JSX into components or define them in a dictionary in the render.
# Wait, replacing complex JSX with regex is dangerous. Let's just create the dictionary of cards and render them.

# First, define the content of the cards.

metrics_code = """
  const metricsComponents = {
    'metric-revenue': (
      <SortableItem id="metric-revenue" key="metric-revenue" className="h-full flex">
    ...
    'metric-active': (
      <SortableItem id="metric-active" key="metric-active" className="h-full flex">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col w-full">
          ...
          <div className="mt-2">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight flex items-baseline">
              {totalActive} <span className="text-sm font-medium text-slate-400 ml-1">/25 Units Active</span>
            </h2>
          </div>
          <div className="mt-auto pt-4">
            ...
            <div className="flex justify-between items-center text-[10px]">
              <div className="flex items-center gap-3">
                <span className="flex items-center text-slate-500"><span className="w-2 h-2 rounded-full bg-emerald-500 mr-1"></span> 15 Good</span>
                <span className="flex items-center text-slate-500"><span className="w-2 h-2 rounded-full bg-amber-400 mr-1"></span> 5 Warn</span>
                <span className="flex items-center text-slate-500"><span className="w-2 h-2 rounded-full bg-red-500 mr-1"></span> 3 Crit</span>
              </div>
              <Link to="/units" className="text-emerald-600 font-semibold hover:underline relative z-20">View Fleet</Link>
            </div>
          </div>
        </div>
      </SortableItem>
    ...
    'metric-pending': (
      <SortableItem id="metric-pending" key="metric-pending" className="h-full flex">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col w-full">
          <div className="flex justify-between items-start mb-2">
          ...
          <div className="mt-2">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight flex items-baseline">
              12 <span className="text-sm font-medium text-slate-400 ml-1">tasks waiting</span>
            </h2>
          </div>
    ...
    'metric-users': (
      <SortableItem id="metric-users" key="metric-users" className="h-full flex">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col w-full">
          <div className="flex justify-between items-start mb-2">
          ...
          <div className="mt-auto pt-5 flex justify-between items-center">
            <div className="flex -space-x-2">
              ...
              <div className="w-6 h-6 rounded-full bg-slate-200 border border-white flex items-center justify-center text-[9px] text-slate-600 font-bold">+2</div>
            </div>
    ...
    'metric-alerts': (
      <SortableItem id="metric-alerts" key="metric-alerts" className="h-full flex">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col w-full">
          <div className="flex justify-between items-start mb-2">
...
# Next we extract the charts code
charts_code = """
  const chartsComponents = {
    'chart-revenue': (
      <SortableItem id="chart-revenue" key="chart-revenue" className="lg:col-span-2 flex h-full">
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col w-full">
    ...
    'chart-fleet': (
      <SortableItem id="chart-fleet" key="chart-fleet" className="flex h-full w-full">
        <div className="flex flex-col gap-6 w-full">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col items-center">
            ...
            <Link to="/maintenance" className="w-full mt-auto bg-slate-50 hover:bg-slate-100 text-slate-700 text-xs font-bold py-3 rounded-xl flex items-center justify-center border border-slate-200 transition-colors relative z-20">
              View Fleet Report & Diagnostics <span className="ml-1">&gt;</span>
            </Link>
          </div>
...
# We need to replace the return statement with the DndContext wrapping everything.
      ...
      <div className="space-y-6">
        ...
        <SortableContext items={metricsOrder} strategy={rectSortingStrategy}>
          <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-5">
            {metricsOrder.map(id => metricsComponents[id])}
          </div>
        </SortableContext>
        ...
        <SortableContext items={chartsOrder} strategy={rectSortingStrategy}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {chartsOrder.map(id => chartsComponents[id])}
          </div>
        </SortableContext>
      </div>
    </DndContext>
...
# Let's rebuild the file by injecting at the right places.

content_split = content.split('  return (')

if len(content_split) == 2:
    new_content = content_split[0] + metrics_code + charts_code + dnd_wrapper + "\n};\n\nexport default SuperAdminDashboard;\n"
    with open('frontend/src/pages/SuperAdminDashboard.jsx', 'w') as f:
        f.write(new_content)
else:
    print("Could not split by '  return (' properly.")
